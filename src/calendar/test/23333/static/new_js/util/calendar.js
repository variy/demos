(function(win,utilities) {
    Bazi.calCtrlBar = function(opts) {
        if(opts.oBoxEl.length === 0){
            return {error:'oBoxEl不存在'}
        }
        this.options = opts;
    ...
    Bazi.calCtrlBar.prototype = {
        ...
        initEvent: function() {
            ...
            this.removeCalendarEl = function(){
                calendar.removeCalendarEl.call(calendar);
            };
    ...
    Calendar.prototype = {
        ...
        init: function(cb) {
            var me = this;
            this.$el = $(Bazi.tpls.tpls_calendar);
            
            // 获得同源顶级窗口
            var top = Bazi.getTopWindow();
            if(top != window.self){
                this.isInIframe = true;
                var iframemMarkId = _.uniqueId('ifmark');
                window.self.document.documentElement.setAttribute('data-iframemark', iframemMarkId);
                var len = top.document.body.getElementsByTagName('iframe').length;
                var theIframe;
                for( var i=0; i<len; i++){
                    if( top.document.body.getElementsByTagName('iframe')[i].contentDocument.documentElement.getAttribute('data-iframemark') === iframemMarkId){
                        theIframe = top.document.body.getElementsByTagName('iframe')[i];
                    }
                }

                this.iframeEl = $(theIframe);

                this.getPositionInIframe();
                top.document.body.appendChild(this.$el[0]);

            }else{
                this.$el.appendTo(this.oBox);
            }
            
            var zindex = Bazi.pop.zIndex || 9999;
            this.$el.css('z-index', ++Bazi.pop.zIndex)
        ...
        getPositionInIframe: function(){
            var iLeft = this.oBox.find('.cal-lunar-info').offset().left;
            var oBoxTop = this.oBox.offset().top;
            var iframeLeft = this.iframeEl.offset().left;
            var iframeTop = this.iframeEl.offset().top;
            var iWidth = 400;
            this.$el.css({
                'left': iLeft + iframeLeft,
                'top': oBoxTop + iframeTop + 64
            });
        },
        bindEvent: function() {
            var self = this;
        ...
        showPannel: function(type) {
            if( ('isInIframe' in this) && this.isInIframe ){
                this.getPositionInIframe();
            }
            this.$el.show();
            this.el.tops.show();
